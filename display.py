...
if __name__ == '__main__':
    ...
    test = TestScreen(list(optotypes.values())[cur_optotype], DISTANCE, 5, cur_logMAR, display)
            ...
            if state == 'idle' or state == 'result':
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_d:
                        ...
                        update_screen = True
        if state == 'idle':
            msg_renderer.render('Visual Acuity Testing Booth', 'Please put on the glasses to start')
            ...
            if cur_pointed >= len(optotypes):
                if set_corrected >= 4:
                    if set_corrected >= 5 and cur_logMAR >= 0.2:
